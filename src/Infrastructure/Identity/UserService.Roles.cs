...
internal partial class UserService
{
    ...
    public async Task<string> AssignRolesAsync(string userId, UserRolesRequest request, CancellationToken cancellationToken)
    {
        ...
        if (await _userManager.IsInRoleAsync(user, FSHRoles.Teacher)
            && request.UserRoles.Any(a => !a.Enabled && a.RoleName == FSHRoles.Teacher))
        {
            // Get count of users in Admin Role
            int adminCount = (await _userManager.GetUsersInRoleAsync(FSHRoles.Teacher)).Count;
